function numberToChinese(num) {
  if (typeof num === 'number') num = num.toString();
    ...
    for (let i = 0; i < num.length; ++i) {
      if (
        num[i].charCodeAt() > '9'.charCodeAt() ||
        num[i].charCodeAt() < '0'.charCodeAt()
      ) {
  ...
  const part4 = (n, isSuffix) => {
    if (n < 10) {
      ...
      if (!isSuffix && Math.floor(n / 10) === 1)
        return '十' + (n % 10 ? map[n % 10] : '');
      else
        return (
          (isSuffix ? '零' : '') +
          map[Math.floor(n / 10)] +
      ...
      if (h) result += map[h] + '百';
      else if (result.length && result[result.length - 1] !== '零')
        result += '零';
  ...
  const part8 = (n, isSuffix) => {
    ...
    if (n < 10000) result = part4(n);
      ...
      result =
        part4(Math.floor(n / 10000), isSuffix) + '万' + part4(n % 10000, true);
    return result;
  };
